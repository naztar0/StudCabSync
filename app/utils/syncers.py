...
class Syncers:
    def __init__(self, app_id: int, user: utils.Student):
        self.app_id = app_id
        self.user = user

    def get_user_profile(self) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 1})

    def get_user_record_book(self, semester: int) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 2, 'semester': semester}, student=self.user)

    def get_user_debts(self) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 3}, student=self.user)

    def get_user_syllabus(self, semester: int) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 4, 'semester': semester}, student=self.user)

    def get_user_rating(self, semester: int) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 5, 'semester': semester}, student=self.user)

    def get_user_payments(self) -> dict | None:
        return utils.api_request(self.app_id, {'email': self.user.email, 'pass': self.user.password, 'page': 6}, student=self.user)

    def sync_profile(self) -> bool:
        user_api = self.get_user_profile()
        if not user_api:
            return False
        train_form, train_level, payment = utils.profile_convert_to_enums(user_api)

        self.user.first_name = user_api['imya']
        self.user.last_name = user_api['fam']
        self.user.middle_name = user_api['otch']
        self.user.year = user_api['kurs']
        self.user.train_form = train_form
        self.user.train_level = train_level
        ...
        with DatabaseConnection() as db:
            ...
            cursor.executemany(
                ...
                [(self.user.first_name, self.user.last_name, self.user.middle_name, self.user.year, self.user.train_form,
                  self.user.train_level, self.user.payment, self.user.id)]
            )
            conn.commit()
        return True
    ...
    def sync_syllabus(self):
        for sem in range(1, 13):
            ...
            syllabus_api = self.get_user_syllabus(sem)
            if not syllabus_api:
                continue
            with DatabaseConnection() as db:
                conn, cursor = db
                ...
                if int(subj['subj_id']) not in [x[1] for x in syllabus]:
                    with DatabaseConnection() as db:
                        ...
                        cursor.execute(
                            ...
                            [self.user.group_id, int(subj['subj_id']), Decimal(subj['credit'] or 0), control, individual_task, Decimal(subj['audit'] or 0), sem]
                        )
                        conn.commit()
                    continue
                for s in syllabus:
    ...
    def get_record_book_syllabus_subjects(self, semester):
        query = "SELECT record_books.id, record_books.syllabus_id, record_books.mark_id, record_books.mark, record_books.debt_date, record_books.user_id, " \
                ...
                "WHERE syllabuses.group_id=(%s) AND syllabuses.semester=(%s)"
        params = [self.user.group_id, semester]
        with DatabaseConnection() as db:
            conn, cursor = db
    ...
    def sync_record_book(self):
        ...
        for sem in range(1, 13):
            logging.debug(f'Semester: {sem}')
            record_book_api = self.get_user_record_book(sem)
            if not record_book_api:
                continue
            result = self.get_record_book_syllabus_subjects(sem)
                    ...
                    for s in result:
                        if s.subject_title.split()[0] == 'Дисципліна':
                            ...
                            with DatabaseConnection() as db:
                                ...
                                conn.commit()
                            break
                record_book_subject_names = [x.subject_title for x in result if x.record_book_id and x.record_book_user_id == self.user.id]
                debt_date = None
                try:
                        ...
                        if s.subject_title == subject_name:
                            ...
                            with DatabaseConnection() as db:
                                ...
                                cursor.execute(
                                    ...
                                    [self.user.id, s.syllabus_id, subj['oc_id'], subj['oc_bol'], debt_date]
                                )
                                conn.commit()
                            break
                result = self.get_record_book_syllabus_subjects(sem)
                for s in result:
                    if s.record_book_user_id == self.user.id and s.subject_title == subject_name:
                        if s.record_book_mark_id != subj['oc_id'] or s.record_book_mark != subj['oc_bol'] or s.record_book_debt_date != debt_date:
                            with DatabaseConnection() as db:
                                ...
                                cursor.execute(
                                    ...
                                    [self.user.id, notifications['subject_mark_changed'][self.user.locale]
                                     .format(subject=s.subject_title, mark=subj['oc_bol'])],
                                )
                                conn.commit()
                        name = utils.Name()
                        ...
                        if name:
                            if not s.subject_professor_name:
                                with DatabaseConnection() as db:
                                    ...
                                    cursor.execute(
                                        "INSERT INTO notifications (user_id, text) VALUES (%s, %s)",
                                        [self.user.id, notifications['subject_professor_changed'][self.user.locale]
                                         .format(subject=s.subject_title, professor=name)],
                                    )
                                    conn.commit()
    ...
    def sync_rating(self):
        ...
        for sem in range(1, 13):
            rating_api = self.get_user_rating(sem)
            if not rating_api:
                continue
            with DatabaseConnection() as db:
                conn, cursor = db
                    ...
                    if user_id:
                        ...
                        with DatabaseConnection() as db:
                            ...
                            if user_id == self.user.id:
                                cursor.execute(
                                    "INSERT INTO notifications (user_id, text) VALUES (%s, %s)",
                                    [user_id, notifications['rating_mark_added'][self.user.locale].format(semester=sem, mark=mark)],
                                )
                            conn.commit()
                    else:
                    ...
                    if rating[1] != mark or rating[2] != comment or user_id != rating[5]:
                        ...
                        with DatabaseConnection() as db:
                            ...
                            if user_id == self.user.id:
                                cursor.execute(
                                    "INSERT INTO notifications (user_id, text) VALUES (%s, %s)",
                                    [user_id, notifications['rating_mark_changed'][self.user.locale].format(semester=sem, mark=mark)],
                                )
                            conn.commit()

    def sync_payments(self):
        payments_api = self.get_user_payments()
        if not payments_api:
            return
        with DatabaseConnection() as db:
            conn, cursor = db
                ...
                with DatabaseConnection() as db:
                    ...
                    if not cursor.fetchone()[0]:
                        ...
                        cursor.execute(
                            ...
                            [int(payment['dog_id']), payment['dog_name'], int(payment['dog_price']), self.user.id, created_at]
                        )
                        conn.commit()
                    ...
                    cursor.execute(
                        "INSERT INTO notifications (user_id, text) VALUES (%s, %s)",
                        [self.user.id, notifications['contract_payment_added'][self.user.locale].format(amount=payment['paid_value'])],
                    )
                    conn.commit()
                continue
        if not payments:
        ...
        if int(payment['dog_price']) != payments[0][1]:
            ...
            with DatabaseConnection() as db:
                ...
                cursor.execute(
                    "INSERT INTO notifications (user_id, text) VALUES (%s, %s)",
                    [self.user.id, notifications['contract_price_changed'][self.user.locale].format(price=int(payment['dog_price']))],
                )
                conn.commit()
